...
### 坑 8：鼠标高速移动时轨迹粒子呈点状分离
*   **现象**：鼠标快速移动时，彩虹尾迹会断裂散落成许多个独立的小球，无法呈现连贯一体的丝滑缎带效果。
*   **原因**：
    由于每一帧的渲染和鼠标位置更新是离散发生的，一旦鼠标移动速度变快，两帧之间的物理位移可能瞬间达到数百像素。如果只在帧渲染点画圆，点与点之间的物理空间就会产生巨大空缺。
*   **解决**：
    在更新 `trail` 队列时，我们引入了线性插值（Linear Interpolation）算法。在推入新的鼠标点前，先计算当前点与上一帧点的距离。如果距离大于设定的间距（`3.0` 像素），则在线段上利用 CPU 均匀内插中间粒子点，并同时赋予当前的生命值时间戳。同时，将 HSL 颜色渐变的色调（hue）分母由固定的 `30.0` 改为动态的 `trail.len() as f32`。这不仅消除了粒子断层，还使得彩虹色能够自适应地、完美地铺满整条移动尾迹，实现了高刷下极致丝滑的缎带特效。
